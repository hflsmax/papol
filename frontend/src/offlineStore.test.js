...
test('queued notes are projected over a cached paper', () => {
  const paper = { uuid: 7, title: 'Paper', comments: [{ uuid: 1, content: 'old' }] };
  const operations = [
    { method: 'POST', path: '/papers/7/comments', body: raw({ content: 'new' }), optimistic: { uuid: -2, content: 'new' } },
    { method: 'PUT', path: '/comments/1', body: raw({ content: 'edited' }), optimistic: { uuid: 1, content: 'edited' } },
  ...
  assert.deepEqual(applyOfflineQueue('/papers/7', paper, operations).comments, [
    { uuid: 1, content: 'edited' },
  ]);
});

test('queued ink is projected over its cached edition', () => {
  const operations = [
    { method: 'POST', path: '/editions/3/ink', body: raw({ points: [1] }), optimistic: { uuid: -4, points: [1] } },
    { method: 'PUT', path: '/ink/-4', body: raw({ points: [2] }), optimistic: { uuid: -4, points: [2] } },
  ];
...
test('queued board operations are projected over the cached board', () => {
  const board = { uuid: 'mine', name: 'Reading', items: [], staged_items: [], groups: [] };
  const operations = [
    { method: 'POST', path: '/boards/mine/comments', body: raw({ content: 'note' }), optimistic: { uuid: -1, kind: 'comment', content: 'note' } },
    { method: 'PUT', path: '/board-items/-1', body: raw({ x: 40, y: 20 }), optimistic: { uuid: -1, x: 40, y: 20 } },
    { method: 'POST', path: '/boards/mine/groups', body: raw({ title: 'Set' }), optimistic: { uuid: -2, title: 'Set' } },
  ];
  assert.deepEqual(applyOfflineQueue('/boards/mine', board, operations), {
    uuid: 'mine', name: 'Reading', staged_items: [],
    items: [{ uuid: -1, kind: 'comment', content: 'note', x: 40, y: 20 }],
    groups: [{ uuid: -2, title: 'Set' }],
  });
});

test('offline-created boards appear in the viewer board picker', () => {
  const operations = [
    { method: 'POST', path: '/boards', body: raw({ name: 'Offline board' }), optimistic: { uuid: 'offline-one', name: 'Offline board' } },
    { method: 'PUT', path: '/boards/existing', body: raw({ name: 'Renamed' }), optimistic: { uuid: 'existing', name: 'Renamed' } },
    { method: 'DELETE', path: '/boards/gone', body: raw(null), optimistic: { queued: true } },
  ];
  assert.deepEqual(applyOfflineQueue('/boards', [
    { uuid: 'existing', name: 'Old name' },
    { uuid: 'gone', name: 'Gone' },
  ], operations), [
    { uuid: 'existing', name: 'Renamed' },
    { uuid: 'offline-one', name: 'Offline board' },
  ]);
});
