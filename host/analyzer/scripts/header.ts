...
// field the rules read, scored against the paper's row in production — title, authors, year, journal, DOI as the
...
// authors as a list, journal, year, doi). docs/rule-analyzer.md says how to
...
for (const want of truth) {
  ...
  const ours = score(rules, got, want);
  const row = ["title", "authors", "year", "journal", "doi"].map((f) => mark(ours[f])).join(" ");
...
console.log("\nfield     rules   (of papers whose row has the field)");
for (const f of Object.keys(rules)) {
  ...
  console.log(`${f.padEnd(9)} ${pct(rules[f])}`);
}
