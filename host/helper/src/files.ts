// A paper's PDF, fetched by the helper itself from the bucket's public
...
// helper cannot be pointed at any other URL, and whatever it reads is
...
// given up on well before GROBID's own five minutes.
...
export async function fetchPaper(address: { url: URL; sha256: string }, maxBytes: number, fetchImpl: typeof fetch = fetch): Promise<Uint8Array> {
  ...
  try {
    ...
    response = await fetchImpl(address.url, { headers: { "user-agent": "papol-helper" }, signal: AbortSignal.timeout(TIMEOUT_MS) });
  } catch (error) {
