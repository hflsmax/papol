...
import { joinTextPieces, strokeBounds, strokeContains, textUnderStrokes } from './paintText.js';
...
test('a flat annotation covers what its swept nib covers, and nothing past it', () => {
  const band = stroke([[100, 105], [200, 105]]);
  assert.equal(strokeContains(band, { x: 150, y: 105 }, PAGE), true);
  assert.equal(strokeContains(band, { x: 150, y: 109.5 }, PAGE), true, 'within half the weight');
  assert.equal(strokeContains(band, { x: 150, y: 111 }, PAGE), false, 'past half the weight');
  assert.equal(strokeContains(band, { x: 201, y: 105 }, PAGE), true, 'within the nib beyond the last point');
  assert.equal(strokeContains(band, { x: 203, y: 105 }, PAGE), false);
});

test('a flat annotation drawn on a slant sweeps its rectangle along the slant', () => {
  const slant = stroke([[100, 100], [200, 200]]);
  assert.equal(strokeContains(slant, { x: 150, y: 150 }, PAGE), true);
  assert.equal(strokeContains(slant, { x: 150, y: 158 }, PAGE), false, 'the nib is tall, not wide');
  assert.equal(strokeContains(slant, { x: 150, y: 154 }, PAGE), true);
});

test('a round annotation is a capsule round its path, and a dot is a disc', () => {
  const round = stroke([[100, 100], [200, 100]], { shape: 'round', width: 20 / 600 });
  assert.equal(strokeContains(round, { x: 205, y: 100 }, PAGE), true, 'the round end reaches past the point');
  assert.equal(strokeContains(round, { x: 150, y: 111 }, PAGE), false);
  const dot = stroke([[300, 300]], { shape: 'round', width: 20 / 600 });
  assert.equal(strokeContains(dot, { x: 307, y: 307 }, PAGE), true);
  assert.equal(strokeContains(dot, { x: 308, y: 308 }, PAGE), false);
});

test('the bounds of an annotation contain everything it paints', () => {
  const band = stroke([[100, 105], [200, 105]]);
  const bounds = strokeBounds(band, PAGE);
  const slack = 1e-9;
...
test('painting a selection and asking the paint for its text gives the selection back', () => {
  ...
  const { text } = textUnderStrokes(characters, bands, pages);
...
test('a freehand highlight takes the words whose middles it covers', () => {
  ...
  assert.equal(textUnderStrokes(characters, [highlight], pages).text, 'keys are');
});
...
test('a hand that clips the words either side takes only the words it was over', () => {
  ...
  assert.equal(textUnderStrokes(characters, [highlight], pages).text, 'for the');
});

test('a word mostly under the annotation is taken whole', () => {
  ...
  assert.equal(textUnderStrokes(characters, [highlight], pages).text, 'secure key');
});
...
test('an underline below the words is not over them, so it has no text', () => {
  ...
  assert.equal(textUnderStrokes(characters, [underline], pages).text, '');
});

test('an annotation only takes text from its own page', () => {
  const characters = [...line('first page', { page: 1 }), ...line('second page', { page: 2, span: 0 })];
  const band = stroke([[40, 105], [40 + 10 * 6, 105]], { page: 2 });
  const { text, bands } = textUnderStrokes(characters, [band], pages);
...
test('pieces are joined with spaces, line breaks and page breaks as a user would copy them', () => {
