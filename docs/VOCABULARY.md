...
| **Demo** | A fictional Papol backed by a disposable session; changes never reach permanent content. The URL determines whether it is on. | `shared/demo.js`; [architecture](demo-architecture.md) — dropped on the Worker (docs/cloud-migration.md, phase 4). Supported features use real handlers; others report that they are unsupported. |
