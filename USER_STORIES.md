...
- **US-7.31** **Papol cannot tell whether the app is installed, and does not pretend to.** The offer is an attempt: the document is asked for, and if it has not opened a moment later, nothing did. The bar then says so in the plainest terms — *Papol for Mac isn't installed* — and turns into the download. It is never phrased as my mistake, and never as an error, because being asked for software I do not have is not a failure on my part.
- **US-7.32** **The download remembers what I was opening.** Once the app is installed, the same bar is still there on the same paper, and one click hands it over — I never have to find my way back to the paper to finish what I started. Until then the tab reads exactly as it did before I asked, and nothing about the offer is repeated at me on the next page.
