...
- **US-7.36** As a user reading a citation's card, I am told **how often this paper cites that work** — *Cited 4 times in this paper*, or *Cited only here* — and the card's ↑ and ↓ buttons take me to the previous and next place it is cited, without closing the card. Knowing what [12] is, the next question is what else the authors say about it, and the answer is scattered across the paper.
- **US-7.37** Stepping is **an exploration, not a move**. From the first step the card says so, and so does the page: it names where I am (*2 of 4 · page 7*) and where I came from, and it holds the way out in plain sight. The steps themselves are not written into the paper's history — no return pill, nothing for [ to walk back through step by step — because I have not gone anywhere yet.
- **US-7.38** **Esc ends the exploration where it began**: the view returns to exactly where I was reading, and the card to the marker I clicked. *Back to page 3* on the card does the same, and so does its ×. If what I found is where I want to read, **Stay here** ends the exploration where I am, and so does **clicking away** — a press on the page, or on another citation — because that click is aimed at what is on screen now, and taking the page away from under it would be worse than any lost place. Ending away from where I began is a move, so it is kept as one: *Back to page 3* is on the return pill, [ takes me back to where I was reading and ] brings me to where I stayed, as after following a link (US-7.11). The steps in between are not in that history; the whole exploration is one move.
- **US-7.39** Each step brings the next marker to where the last one was: the page moves, **the card stays put** under my pointer, and the marker it now belongs to is lit. The sentence around it is on screen, because that sentence is what I came for.
- **US-7.40** The steps run in **reading order** — down one column and on to the next — and past the last place they come round to the first, as a search does. The count says where I am, so coming round is never a surprise.
- **US-7.41** A marker that cites several works ("[3–5]") steps through the places that cite **the work the card is showing**; choosing another work in its range changes which places those are. A place where the work is cited alongside others ("[2, 7, 9]") counts as a place it is cited, and one matched only by counting its number is marked as the guess it is (US-7.9).
...
- **US-7.27** I decide, and I decide once. **Not now** puts the offer away for this document; **Don't ask again** retires it for this browser; clearing the browser's data for Papol brings it back. There is no standing answer to give: the app is never opened without my asking on the document in front of me, so nothing can be set going that I then have to find the way to stop.
