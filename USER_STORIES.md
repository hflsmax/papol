...
- **US-7.32** **The address I am already at is the handoff.** Papol for Mac claims its own web addresses, so `https://mc-pony.com/papol/viewer/…` opens in the app for a reader who has it and in the browser for everyone else — one link, no second vocabulary, and nothing to keep in step. A private `papol://` address stands behind it for the cases the system has not yet associated: a notarized app that has never been launched is not yet trusted with its own domain, so the first handoff after installing may go the private way.
- **US-7.33** **Where macOS already offers this, Papol does not ask twice.** Safari puts its own "open in the app" affordance on an address the app claims, and two offers for one paper is worse than one. Papol's bar yields to the system's.
